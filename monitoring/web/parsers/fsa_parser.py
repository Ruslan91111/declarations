...
class FSADeclParser(BaseParser):
    ...
    def input_doc_numb(self):
        ...
        self.browser_worker.input_and_press_search(
    ...
    def return_for_input_doc_numb(self, xpath=FsaXPaths.RETURN_BACK_DECL.value):
        ...
        self.browser_worker.wait_and_click_elem(xpath)

    def click_chapter(self, item: int) -> str:
        """ Переключить подраздел и вернуть его наименование. """
        try:
            chapter = self.browser_worker.wait_and_click_elem(
                self.template_for_chapter_xpath + f'[{item}]/a')
        except ElementClickInterceptedException:
            chapter = self.browser_worker.wait_elem_clickable(
                self.template_for_chapter_xpath + f'[{item}]/a')
            self.browser_worker.browser.execute_script(
                "arguments[0].scrollIntoView(true);", chapter)
            chapter.click()
    ...
    def get_last_chapter_numb(self) -> int:
        ...
        elements = self.browser_worker.wait_all_located_by_xpath(
    ...
    def get_data_from_chapter(self, data: dict, chapter_name: str) -> dict:
        ...
        headers = self.browser_worker.find_all_elems_by_class("info-row__header")
        texts = self.browser_worker.find_all_elems_by_class("info-row__text")
    ...
    def save_status_from_site(self):
        ...
        self.document.status_on_site = (self.browser_worker.get_text_by_xpath(
            FsaXPaths.DOC_STATUS.value))

    def check_403_error(self) -> bool:
        """ Проверка нет ли на экране сообщения об ошибке 403."""
        return bool(self.browser_worker.find_all_elems_by_xpath(FsaXPaths.ERROR_403.value))
    ...
    def check_not_available_error(self):
        ...
        while time.time() - start < 2:
            ...
            error = self.browser_worker.find_all_elems_by_xpath(
                FsaXPaths.SERV_NOT_AVAILABLE.value)
    ...
    def is_doc_number_loaded(self, timeout=5) -> bool:
        ...
        while time.time() - start < timeout:
            loaded_number_on_site = self.browser_worker.get_text_by_xpath(
    ...
    def no_matching_records(self):
        """ Проверить прогрузились ли документы на новый введенный номер документа. """
        no_records_matching_the_search = self.browser_worker.get_text_by_xpath(
    ...
    def _update_document_status(self, row):
        ...
        image_with_status = self.browser_worker.wait_elem_clickable(
    ...
    def _scroll_to_next_row(self, row):
        ...
        next_row_on_page = self.browser_worker.wait_elem_clickable(
    ...
    def click_correct_doc(self, row) -> bool:
        ...
        fsa_doc_numb = self.browser_worker.wait_elem_clickable(
            FsaXPaths.ROW_DOC_ON_SITE.value.format(row=row, column=3))
        text_fsa_doc_numb = fsa_doc_numb.text
        fsa_expiration_date = self.browser_worker.get_text_by_xpath(
    ...
    def search_correct_document(self) -> bool:
        ...
        count_of_rows = len(
            self.browser_worker.find_all_elems_by_xpath(FsaXPaths.COUNT_OF_PAGE.value))
    ...
    def check_not_valid_status(self) -> bool:
        ...
        fsa_doc_number = self.browser_worker.get_text_by_xpath(
            FsaXPaths.ROW_DOC_ON_SITE.value.format(row=2, column=3))
        status_on_page = self.browser_worker.wait_elem_clickable(
...
class FSACertParser(FSADeclParser):
    ...
    def get_inner_elements_from_page(self, chapter: str) -> dict:
        ...
        inner_elements = {}
        self.browser_worker.wait_all_located_by_class('card-edit-row__content')
        keys = self.browser_worker.find_all_elems_by_class('card-edit-row__header')
        values = self.browser_worker.find_all_elems_by_class('card-edit-row__content')
